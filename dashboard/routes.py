...
def update_avatar():
    ...
    if not avatar_filename:
        return jsonify({"success": False, "message": "Avatar seçimi geçersiz."}), 400
...
@login_required
def avatars_page():
    avatar_path = os.path.join(dashboard_bp.root_path, '..', 'static', 'img', 'avatars')
    available_avatars = []
    if os.path.exists(avatar_path):
        available_avatars = [f for f in os.listdir(avatar_path) if os.path.isfile(os.path.join(avatar_path, f))]

    return render_template('partials/_avatars.html', 
                           user=current_user, 
